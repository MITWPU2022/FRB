...
Python class and command line utility for reading and plotting search_FRB files.

This provides a class, search_FRB(), which can be used to read a FRB file (.fil or .h5):


    fil = search_FRB("test_psr.fil")
    print(fil.header)
...
from FRB.io import file_wrapper as fw
...
class search_FRB():
    """ Class for loading and writing FRB data (.fil, .h5) """

    def __repr__(self):
        return "search_FRB data: %s" % self.filename
    ...
    def __init__(self, filename=None, f_start=None, f_stop=None, t_start=None, t_stop=None,
        ...
        """ Class for loading and plotting FRB data.

        This class parses the FRB file and stores the header and data
        as objects:
            fb = search_FRB('filename_here.fil')
            fb.header        # FRB header, as a dictionary
            fb.data          # FRB data, as a numpy array

        Args:
            filename (str): filename of FRB file.
            ...
            header_dict (dict): Create FRB from header dictionary + data array
            data_array (np.array): Create FRB from header dict + data array
        """
    ...
    def get_freqs(self):
        """
        Get the frequency array for this search_FRB object.
    ...
    def grab_data(self, f_start=None, f_stop=None,t_start=None, t_stop=None, if_id=0):
        ...
        if self.container.isheavy():
            raise Exception("search_FRB.grab_data: Large data array was not loaded!  Try instantiating search_FRB with max_load.")

        try:
            self.freqs = self.container.populate_freqs()
        except Exception as ex:
            raise Exception("search_FRB.grab_data: Cannot allocate frequency array") from ex

        try:
            self.timestamps = self.container.populate_timestamps()
        except Exception as ex:
            raise Exception("search_FRB.grab_data: Cannot allocate timestamps array") from ex
...
def cmd_tool(args=None):
    """ Command line tool for plotting and viewing info on FRB files """


    from argparse import ArgumentParser

    parser = ArgumentParser(description="Command line utility for reading and plotting FRB files.")

    parser.add_argument('filename', type=str,
                        help='Name of file to read')
    parser.add_argument('-p', action='store', dest='what_to_plot', type=str,
                        choices=['w', 's', 't', 'k', 'mm', 'a', 'ank'],
                        help='Show: "w" search_FRB (freq vs. time) plot; "s" integrated spectrum plot; \
    ...
    fil = search_FRB(filename, f_start=parse_args.f_start, f_stop=parse_args.f_stop, t_start=parse_args.t_start, t_stop=parse_args.t_stop, load_data=load_data, max_load=parse_args.max_load)
    fil.info()
