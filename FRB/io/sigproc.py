...
#   * data_type (int): 1=FRB; 2=time series... others to be added
...
#   * fch1 (double): centre frequency (MHz) of first FRB channel
#   * foff (double): FRB channel bandwidth (MHz)
...
#   * nchans (int): number of FRB channels
...
def len_header(filename):
    """ Return the length of the FRB header, in bytes
...
def read_header(filename, return_idxs=False):
    """ Read FRB header and return a Python dictionary of key:value pairs
    ...
    with open(filename, 'rb') as fh:
        ...
        try:
            ...
            raise RuntimeError("Not a valid FRB file.")
...
def fix_header(filename, keyword, new_value):
    ...
    Notes:
        This will overwrite the current value of the FRB with a desired
