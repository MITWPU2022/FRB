...
class Reader(object):
    ...
    def adjust_n_coarse_chan(self, n_coarse_chan, nchans):
        ...
        if n_coarse_chan < 1:
            errmsg1 = "FRB:io:base_reader:adjust_n_coarse_chan: n_coarse_chan={}, nchans={}" \
                      .format(n_coarse_chan, nchans)
            logger.warning(errmsg1)
            errmsg2 = "FRB:io:base_reader:adjust_n_coarse_chan: n_coarse_chan < 1. Replacing that with a value of 64 (SWAG)."
        ...
        if n_coarse_chan > nchans: # exceeds the number of fine channels?
            errmsg1 = "FRB:io:base_reader:adjust_n_coarse_chan: n_coarse_chan={}, nchans={}" \
                      .format(n_coarse_chan, nchans)
            logger.warning(errmsg1)
            errmsg2 = "FRB:io:base_reader:adjust_n_coarse_chan: n_coarse_chan > nchans. Replacing that with the value of nchans (SWAG)."
            logger.warning(errmsg2)
            return nchans
    ...
    def calc_n_coarse_chan(self, chan_bw=None):
        ...
        if nchans >= HIRES_THRESHOLD:
            ...
            # Early GBT data has non-2^N FFT length, check if it is GBT data
                ...
                errmsg1 = "FRB:io:base_reader:calc_n_coarse_chan: hires nchans not divisible by 2^20 and not GBT"
                logger.warning(errmsg1)
        ...
        # Not high resolution data and Not GBT
            ...
            errmsg1 = "FRB:io:base_reader:calc_n_coarse_chan: not hires and not GBT. Setting a value of 1"
            logger.warning(errmsg1)
            errmsg2 = "Setting a value of 64 (SWAG). In turbo_seti, you can specify n_course_chan explicitly."
            logger.info(errmsg2)
            return 64
