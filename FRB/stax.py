...
r"""
Make search_FRB plots of a file set, view from top to bottom.
"""
...
def plot_dynamic_spectrum(wf, f_start=None, f_stop=None, **kwargs):
    ...
    Parameters
    ----------
    wf : FRB.search_FRB object
        search_FRB object of an H5 or Filterbank file containing the dynamic spectrum data.
    f_start : float
    ...
    Notes
    -----
    Plot a single-panel search_FRB plot (frequency vs. time vs. intensity)
    ...
    # Make sure search_FRB plot is under 4k*4k
    ...
    try:
        ...
        print("search_FRB info for {}:".format(wf.filename))
        wf.info()
...
def make_waterfall_plots(file_list, plot_dir, plot_dpi, height_ratios, f_start=None, f_stop=None, **kwargs):
    ...
    wf = bl.search_FRB(file_list[0], load_data=False)
    ...
    for ii, filename in enumerate(file_list):
        ...
        wf = bl.search_FRB(filename, f_start=the_lowest, f_stop=the_highest)
...
def cmd_tool(args=None):
    ...
    parser = ArgumentParser(description='Make search_FRB plots from a set of files, viewed from top to bottom.')
    ...
    for file in args.file_list:
        wf = bl.search_FRB(file, max_load=1)
